...
function updateMainDashboard() {
    ...
    if ((menuGrid || reservationWidget) && currentUser && currentUser.type === 'teacher') {
        ...
        setTimeout(() => {
            // 예약 상태 새로 조회하여 확실히 최신 데이터 반영
            const reservationStatus = getUserReservationStatus(currentUser);
            console.log('🔄 대시보드 업데이트용 예약 상태:', reservationStatus);

            // 기존 예약 위젯만 업데이트 (전체 화면 다시 그리기 대신)
            const currentWidget = document.querySelector('.reservation-widget');
            if (currentWidget) {
                const createReservationWidget = (computerReservations, routerReservations, libraryReservations) => {
                    console.log('🎨 위젯 업데이트 중:', { computerReservations, routerReservations, libraryReservations });

                    // 위젯 내용 업데이트 로직은 goBack() 함수의 createReservationWidget과 동일
                    function getCellStatus(dayParam, period, dateStr, allReservations) {
                        const date = new Date(dateStr);
                        const dayOfWeek = date.getDay();
                        const actualDayNames = ['일', '월', '화', '수', '목', '금', '토'];
                        ...
                        const existingReservation = allReservations.find(req => {
                            return req.useDate === dateStr && req.period === period;
                        });

                        if (existingReservation &&
                            existingReservation.requester === currentUser.name &&
                            existingReservation.requesterGrade == currentUser.grade &&
                            existingReservation.requesterClass == currentUser.class) {
                            const displayText = existingReservation.crossGradeReservation ? '내 예약(협의)' : '내 예약';
                            return {
                            ...
                            const displayText = existingReservation.crossGradeReservation && existingReservation.displayName ?
                                existingReservation.displayName : `${existingReservation.requesterGrade}학년 예약됨`;
                            return {
                                status: 'reserved',
                                content: displayText,
                                clickable: false
                        ...
                        return { status: 'available', content: '가능', clickable: true };
                    }
                    ...
                    nextWeekEnd.setDate(thisMonday.getDate() + 11);

                    const generateWeeklyTable = (facilityReservations, facilityName, facilityClass) => {
                        const timeSlots = [
                            { period: '1교시', time: '09:10-09:50' },
                            { period: '2교시', time: '10:00-10:40' },
                            { period: '3교시', time: '10:50-11:30' },
                            { period: '4교시', time: '11:40-12:20' },
                            { period: '5교시', time: '13:20-14:00' },
                            { period: '6교시', time: '14:10-14:50' }
                        ];
                        ...
                        timeSlots.forEach(slot => {
                            ...
                            tableHTML += `<div class="time-slot">${slot.period}<br><small>${slot.time}</small></div>`;

                            days.forEach((day, dayIndex) => {
                                const currentDate = new Date(thisWeekStart);
                                currentDate.setDate(thisWeekStart.getDate() + dayIndex);
                                const dateStr = currentDate.toISOString().split('T')[0];

                                if (currentDate >= thisWeekStart && currentDate <= nextWeekEnd) {
                                    const cellStatus = getCellStatus(day, slot.period, dateStr, facilityReservations);
                                    const statusClass = cellStatus.status;

                                    tableHTML += `<div class="schedule-cell ${statusClass} ${facilityClass}">${cellStatus.content}</div>`;
                                } else {
                                    tableHTML += `<div class="schedule-cell disabled">-</div>`;
                                }
                            });
                        ...
                        tableHTML += `<div class="week-divider">다음주</div>`;
                        ...
                        timeSlots.forEach(slot => {
                            ...
                            days.forEach((day, dayIndex) => {
                                ...
                                currentDate.setDate(nextWeekStart.getDate() + dayIndex);
                                const dateStr = currentDate.toISOString().split('T')[0];
                    ...
                    const hasAnyReservation = computerReservations.length > 0 || routerReservations.length > 0 || libraryReservations.length > 0;
                    ...
                    return `
                        <div class="reservation-widget ${hasAnyReservation ? 'has-reservations' : ''}">
                            ...
                            <div class="facilities-container">
                                ${generateWeeklyTable(computerReservations, '🖥️ 컴퓨터실', 'computer-reservation')}
                                ${generateWeeklyTable(routerReservations, '📶 공유기 (늘봄교실3)', 'router-reservation')}
                                ${generateWeeklyTable(libraryReservations, '📚 도서관', 'library-reservation')}
                            </div>
                        </div>
                    `;
                };
                ...
                // 위젯 내용 교체
                const newWidgetHTML = createReservationWidget(reservationStatus.computer, reservationStatus.router, reservationStatus.library);
                currentWidget.outerHTML = newWidgetHTML;
                console.log('✅ 예약 위젯만 업데이트 완료');
            } else {
                // 위젯이 없으면 전체 화면 다시 그리기
                goBack();
            }
